...
import Scale from "./Scale";
 const PieGroup = (
   { groupValue, groupValue: { x, y, height, width, dataArray }, isSelected, onSelect, onChange, }) => {
...
//*? -------------------------------------------------------------------------- */
  ...
  return (
    <Fragment>
      <Group x={150} y={100} 
             height={height} width={width} draggable 
             onClick={onSelect} onTap={onSelect} ref={shapeRef}
             onDragEnd={(e) => { onChange({ ...groupValue, x: e.target.x(), y: e.target.y(), }); }}
             onTransformEnd={(e) => {
      ...
      >
        ...
        {dataArray.map((item, idx) => (
          <Pie key={idx} idx={idx} width={(2 * width) / 3} height={height}
            radius={(2 * width) / 3 > height / 2 ? height / 2 : width / 3}
            data={item} total={sum} 
            rotation={getRotation(idx, item)} totalInput={dataArray.length}
          />
        ))}

        {/* //* This will return Scale
         */}
        <Group x={(width / 3) * 2} y={dataArray.length > 6 ? 0 : height / 2} width={width / 3} >
          {dataArray.map((item, idx) => (
            <Scale key={idx} idx={idx} width={width / 3} height={height}
              radius={width / 3 > height ? height / 2 : width / 3}
              data={item} totalInput={dataArray.length}
            />
          ))}
        </Group>
